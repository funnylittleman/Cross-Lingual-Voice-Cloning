...
class Tacotron2Loss(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.ce_loss = nn.CrossEntropyLoss()                                                                        #Maybe use (reduction='sum') ?

    def forward(self, model_output, targets, re, batched_speakers, batched_langs):
        ...
        kl_loss = kld(Normal(means[0], stddevs[0]), re.p_zo_given_yo.distrib_lis[batched_langs[0]]).sum()
        for i, lang in enumerate(batched_langs[1:], 1) :
            kl_loss += kld(Normal(means[i], stddevs[i]), re.p_zo_given_yo.distrib_lis[lang]).sum()
        ...
        kl_loss = kl_loss/batched_langs.shape[0]
